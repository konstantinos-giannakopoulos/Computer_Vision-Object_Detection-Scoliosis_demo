...
def main(image_filename,model_option):
    ...
    #    sys.exit(0)
    net_type = model_option #'mb1-ssd' #sys.argv[1]
    model_path = 'utils/sagittal/mb1-ssd-Epoch-90-Loss-0.04602747387252748.pth' #sys.argv[2]
    label_path = 'utils/sagittal/voc-model-labels.txt' #sys.argv[3]
    ...
    model_path = 'utils/sagittal/mb1-ssd-Epoch-90-Loss-0.04602747387252748.pth'
    model_path = resource_path(model_path)
   ...
   # net = create_resnet50_ssd(len(class_names), is_test=True)
    ...
    #     cv2.rectangle(orig_image, (box[0], box[1]), (box[2], box[3]), (255, 255, 0), 4)
        ...
        cv2.putText(orig_image, label,
                    ...
                    1,  # line type
                    cv2.LINE_AA)
    path = 'sagittal/output_image.jpg'
    path = resource_path(path)
    path = resource_path(path)
    cv2.imwrite(path, orig_image)
    boxes = boxes.numpy()
    boxes = pd.DataFrame(boxes, columns=['x_center', 'y_center', 'width', 'height', 'angle'])
    boxes.to_csv(resource_path(r'sagittal/output_boxes.csv'), index=False)
