...
class FPNSSD(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for i in range(len(features) - 1, -1, -1):
            ...
            if upstream_feature is not None:
                print(self.upsamplers[i])
                print(self.upsamplers[i](upstream_feature).shape)
    ...
    def compute_header(self, i, x):
        ...
        location = location.view(location.size(0), -1, 5)   #Modification  (location.size(0), -1, 4)
    ...
    def init_from_pretrained_ssd(self, model):    ### Modification ### Addition fun init_from_pretrained_ssd()
...
class MatchPrior(object):
    def __init__(self, center_form_priors, center_variance, size_variance, iou_threshold):
        ...
        #self.corner_form_priors = box_utils.center_form_to_corner_form(center_form_priors) ### Modification ###
    ...
    def __call__(self, gt_boxes, gt_labels):
        ...
        boxes, labels = box_utils_v2.assign_priors(gt_boxes, gt_labels,
                                                self.corner_form_priors, self.iou_threshold) ### Modification ###
