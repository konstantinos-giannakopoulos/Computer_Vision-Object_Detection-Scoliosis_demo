...
def create_fpn_mobilenetv1_ssd(num_classes):
    base_net = MobileNetV1(1001).model  # disable dropout layer ### Modification ### .features -> model

    source_layer_indexes = [
        (69, Conv2d(in_channels=512, out_channels=256, kernel_size=1)),    ### (69, Conv2d(in_channels=512, out_channels=256, kernel_size=1)),
        (len(base_net), Conv2d(in_channels=1024, out_channels=256, kernel_size=1)), ### (len(base_net), Conv2d(in_channels=1024, out_channels=256, kernel_size=1)),
    ]
    extras = ModuleList([
        Sequential(
            Conv2d(in_channels=1024, out_channels=256, kernel_size=1),
            ReLU(),
            Conv2d(in_channels=256, out_channels=512, kernel_size=3, stride=2, padding=1),  ### out: 256
            ReLU()
        ),
        Sequential(
            Conv2d(in_channels=512, out_channels=128, kernel_size=1),   ### in: 256
            ReLU(),
    ...
    regression_headers = ModuleList([
        Conv2d(in_channels=512, out_channels=1 * 5, kernel_size=3, padding=1),  ### 256
        Conv2d(in_channels=1024, out_channels=1 * 5, kernel_size=3, padding=1),  ### 256
        Conv2d(in_channels=512, out_channels=1 * 5, kernel_size=3, padding=1),  ### 256
    ...
    classification_headers = ModuleList([
        Conv2d(in_channels=512, out_channels=1 * num_classes, kernel_size=3, padding=1),  ### 256
        Conv2d(in_channels=1024, out_channels=1 * num_classes, kernel_size=3, padding=1),  ### 256
        Conv2d(in_channels=512, out_channels=1 * num_classes, kernel_size=3, padding=1),  ### 256
