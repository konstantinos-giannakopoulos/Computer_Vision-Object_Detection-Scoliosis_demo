...
def draw_datapoints_to_image(datapoints,image_file):
    ...
    for datapoint in datapoints:
        ...
        modified_image = cv2.circle(image, (x,y), radius=33, color=(0, 0, 255), thickness=-1)
    
    final_image = 'frontal/final_output.jpg'
...
def draw_lines_to_image(datapoints_with_slopes):
    image_file = 'frontal/final_output.jpg'
    ...
    final_image = 'frontal/final_final_output.jpg'
    final_image = resource_path(final_image)
    cv2.imwrite(final_image, modified_image)
    print('Final Image saved.')
...
def show_all_angles_to_image(final_datapoints__array):
    image_file = 'frontal/final_final_output.jpg'
    image_file = resource_path(image_file)
    orig_image = cv2.imread(image_file)
    image = cv2.cvtColor(orig_image, cv2.COLOR_BGR2RGB)
    length = 200
...
def report(final_datapoints__array, angles):
    print('\n\n------------ Results ------------')
...
def main(file):
    ...
    image_file = 'frontal/input_image.jpg'
    image_file = resource_path(image_file)
    ...
    # print results
    num_slope_changes, shape, cobb_angles = report(final_datapoints__array, angles)
